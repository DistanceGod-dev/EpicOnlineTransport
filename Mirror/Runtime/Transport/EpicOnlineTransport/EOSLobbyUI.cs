...
public class EOSLobbyUI : EOSLobby {
    ...
    private void DrawLobbyList() {
        ...
        foreach (LobbyDetails lobby in foundLobbies) {
            ...
            var opations1 = new LobbyDetailsCopyAttributeByKeyOptions {AttrKey = AttributeKeys[0]};
            Epic.OnlineServices.Result result;
            result = lobby.CopyAttributeByKey(ref opations1, out lobbyNameAttribute);
            if (result != Result.Success)
            {
                Debug.Log("--------------------------lobby.CopyAttributeByKey------------------------"+result);
            }
            //draw the lobby result
            GUILayout.BeginHorizontal(GUILayout.Width(400), GUILayout.MaxWidth(400));
            //draw lobby name
            GUILayout.Label(lobbyNameAttribute?.Data.Value.Value.AsUtf8.Length > 30 ?  lobbyNameAttribute?.Data.Value.Value.AsUtf8.ToString().Substring(0, 27).Trim() + "..." : lobbyNameAttribute?.Data.Value.Value.AsUtf8, GUILayout.Width(175));
            GUILayout.Space(75);
            //draw player count
            var optation2 = new LobbyDetailsGetMemberCountOptions { };
            GUILayout.Label(lobby.GetMemberCount(ref optation2).ToString());
            GUILayout.Space(75);
    ...
    private void DrawLobbyMenu() {
        ...
        var opations1 = new LobbyDetailsGetMemberCountOptions { };
        //draws players
        for (int i = 0; i < ConnectedLobbyDetails.GetMemberCount(ref opations1); i++) {
