...
namespace EpicTransport {
    ...
    public class EOSSDKComponent : MonoBehaviour {
...
#if UNITY_EDITOR
            var libraryPath = "Assets/Plugins/EOSSDK/Bin/" + Config.LibraryName;
        ...
        protected void InitializeImplementation() {
            ...
            LoggingInterface.SetCallback(delegate(ref LogMessage message) { Logger.EpicDebugLog(message); });
        ...
        private void ConnectInterfaceLogin() {
            ...
            if (connectInterfaceCredentialType == Epic.OnlineServices.ExternalCredentialType.Epic) {
                ...
                var  authTokenOptions = new Epic.OnlineServices.Auth.CopyUserAuthTokenOptions();
                Result result = EOS.GetAuthInterface().CopyUserAuthToken(ref authTokenOptions, localUserAccountId, out token);
            ...
            {
                Type = connectInterfaceCredentialType, Token = connectInterfaceCredentialToken
            };
        ...
        private void OnConnectInterfaceLogin(ref Epic.OnlineServices.Connect.LoginCallbackInfo loginCallbackInfo) {
            if (loginCallbackInfo.ResultCode == Result.Success) {
                ...
                Debug.Log("Login returned " + loginCallbackInfo.ResultCode + "\nRetrying...");
                var  userOptions = new Epic.OnlineServices.Connect.CreateUserOptions()
                    {ContinuanceToken = loginCallbackInfo.ContinuanceToken};
                EOS.GetConnectInterface().CreateUser(ref userOptions, null, (ref Epic.OnlineServices.Connect.CreateUserCallbackInfo cb) => {
